...
class Helper {
  ...
  static buildTestApps(app, suites, tiSdk, moreLogs, done) {
    ...
    function appc(flags, next) {
      ...
      // no fancy stuff
      flags = flags.concat('--no-banner', '--no-services');

      const cmd = spawnConvert(appcExe, flags);

      // assume that the machine has appc cli and is already logged in
      logger.info(`Running: '${cmd} ${flags.join(' ')}' ...`);
    ...
    suites.forEach((targetSuite) => {
      p = p
        .then(() => {
          ...
          const tiProjDir = makeAbs(app, path.join(targetSuite.name, tiProj));
  ...
  static createTests(app, suites) {
    ...
    suites.forEach((targetSuite) => {
      ...
      if (targetSuite.platform === 'ios') {
        ...
        desiredCap.app = makeAbs(app, iosApp);
        ...
        desiredCap.app = makeAbs(app, androidApk);
      ...
      .then(() => {
        return new Promise((resolve) => {
          ...
          fs.watchFile(playerLog, () => {
						const stream = new fs.createReadStream(playerLog);

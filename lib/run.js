...
class Helper {
	...
	*/
	static transform(suiteArg, platformArg) {
		const suites = [];
			...
			return new Promise(resolve => {
				...
				fs.watchFile(playerLog, () => {
					...
					stream.on('data', output => {
						const matches = output.toString().trim().match(/\d+-\d+-\d+T\d+:\d+:\d+\+\d+:\d+ \[Genymotion Player:\d+\] \[debug\] Device booted in \d+ ms/g);
						if (matches) {
							const
								...
								dateTime = lastLine.match(/\d+-\d+-\d+T\d+:\d+:\d+\+\d+:\d+/g);

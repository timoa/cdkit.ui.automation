...
/**
	an internal methodod used strictly by startClient() to register the different
...
class Setup {
  ...
  /**
		...
		@return {Promise} - returns a Promise object; typically, should be called in mocha's before methodod
	*/
  startClient(cap, wLogs) {
    // if logging is enabled, just need to call logging once i.e. register logging events once
    if (wLogs && !this.logOn) {
      this.logOn = true;
      logging(this.driver);
    }
  ...
  /**
		...
		will be used in mocha's after methodod
	*/
  stopClient() {
